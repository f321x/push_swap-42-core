...
/*   Updated: 2023/11/10 09:14:01 by ***REMOVED***            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_print_char_argument(t_pf_data *data)
{
	...
	if (*(data->string_position) != '%')
		ft_putchar_pf_fd(va_arg(*(data->arguments), int), 1, data);
	else
		ft_putchar_pf_fd('%', 1, data);
...
void	ft_print_number_argument(t_pf_data *data)
{
	...
	data->string_position++;
	data->min_width = 0;
	ft_putstr_pf_fd(number_string, 1, data);
	free(number_string);
}
